...
imglib.Image _histogramEqualization(imglib.Image image) {
  ...
  for (var i = 0; i < histogram.length; i++) {
    ...
    mapping[i] = mapped.round().clamp(0, 255).toInt();
  }
  ...
  for (var y = 0; y < image.height; y++) {
    for (var x = 0; x < image.width; x++) {
      final pixel = image.getPixel(x, y);
      final eq = mapping[pixel.r.toInt()];
...
imglib.Image _unsharp(
  ...
  for (var y = 0; y < image.height; y++) {
    for (var x = 0; x < image.width; x++) {
      ...
      final r = _clampChannel(original.r + (original.r - soft.r) * amount);
      final g = _clampChannel(original.g + (original.g - soft.g) * amount);
      final b = _clampChannel(original.b + (original.b - soft.b) * amount);
